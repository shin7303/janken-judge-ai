...
設定は端末の `localStorage` に限定して保存する。旧 `/play/setup` は設定画面へ互換リダイレクト相当の表示を行い、プレイ開始の主導線にはしない。

### 6.3 プレイ画面 `/play`

主な領域は、カメラ映像、左右プレイヤー領域、現在のジェスチャー、カウントダウン、状態メッセージ、最小限の推論状態で構成する。詳細なカメラ診断や設定はこの画面に常設しない。

カメラ権限の要求だけはブラウザ要件に従い明示的なボタン操作で行う。カメラとモデルが準備でき、左右のROIに一人ずつの手が入り、信頼度・FPS・タブ状態の条件を満たして安定した後は、既定で `autoStartDelayMs` 後にカウントダウンを自動開始する。設定で自動開始を無効にした場合だけ、手動の開始ボタンを表示する。

スマートフォンのプレイ画面は、390×844程度の縦表示で主要な操作とライブ映像が一画面に収まる。横向きの短い画面では補助説明を隠し、映像と状態を優先する。
...
3. 既定では750ms待機して自動開始する（設定で無効なら開始ボタンを使う）
4. 3、2、1のカウントダウン
...
| カメラなし | 対応カメラが見つからない | 対応ブラウザ・接続を確認して再試行 |
| モデル読込失敗 | 読込に失敗 | 再試行 |
